...
def plan_simultaneously(x_centroid, y_centroid, x, y, atol, x_target=None, y_target= None):
    ...
    while True:
        if not np.isclose(x_target, x_centroid.get(), atol):
            yield from walk_to_pixel(x_centroid, x, x_target, first_step=0.1, 
                                 target_fields=[x_centroid.name, x.name], tolerance = atol, average = 5, 
                                  system=[y, y_centroid]) 
        elif not np.isclose(y_target, y_centroid.get(), atol):
            yield from walk_to_pixel(y_centroid, y, y_target, first_step=0.1, tolerance = atol, average = 5,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tol', default=3, type = int, help='Set tolerance value')
    ...
    #Executes the plan
    RE(plan_simultaneously(x_centroid, y_centroid, x_motor, y_motor, atol), md={'plan_name': 'special'})
